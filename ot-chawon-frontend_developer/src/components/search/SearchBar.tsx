    ...
    <div className={cn('relative w-full', className)}>
      <div
        className={cn(
          ...
          'bg-[#212121] border border-[#616161] rounded-lg',
      ...
      >
        ...
        <input
          ...
          className={cn(
            'flex-1 bg-transparent text-[#F9F9F9] placeholder:text-[#9E9E9E]',
        ...
        {value && (
          <button
            type="button"
            onClick={handleClear}
            className="flex-shrink-0 w-5 h-5 flex items-center justify-center rounded-full bg-[#616161] hover:bg-[#9E9E9E] transition-colors"
            aria-label="검색어 지우기"
          >
      ...
      {showSuggestions && suggestions.length > 0 && (
        <ul
          className={cn(
            ...
            'bg-[#212121] border border-[#616161] rounded-lg overflow-hidden shadow-lg'
          )}
        >
          {suggestions.map((suggestion, index) => (
            <li key={suggestion}>
              ...
              >
                ...
                <span className={cn('text-sm', index === 0 ? 'text-[#FF6B35] font-medium' : 'text-[#F9F9F9]')}>
                  {suggestion}
                </span>
              </button>
            </li>
